...
document.getElementById('btn-extract-upload').addEventListener('click', function() {
    const front = document.getElementById('front-cccd').files.length;
    const back = document.getElementById('back-cccd').files.length;
    
    if (front === 0 || back === 0) {
        alert("Vui lòng tải lên đầy đủ 2 mặt của CCCD!");
        return;
    }

    const btn = this;
    const originalHtml = btn.innerHTML;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span> Đang trích xuất...';
    btn.disabled = true;
    ...
    setTimeout(() => {
        ...
        const dummyData = {
            "Số CCCD": "079099123456",
            "Họ và tên": "NGUYỄN VĂN A",
            "Ngày sinh": "01/01/1999",
            "Giới tính": "Nam",
            "Quốc tịch": "Việt Nam",
            "Nơi cư trú": "Quận 1, TP. Hồ Chí Minh"
        };
        document.getElementById('raw-data-result').value = JSON.stringify(dummyData, null, 2);
    }, 1500);
});
...
btnCaptureScan.addEventListener('click', function() {
    const btn = this;
    const originalHtml = btn.innerHTML;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span> Đang nhận diện AIVision...';
    btn.disabled = true;
    ...
    // Giả lập gọi API AI Vision
    setTimeout(() => {
        btn.innerHTML = originalHtml;
        btn.disabled = false;
        
        const dummyData = {
            "Số CCCD": "001099887766",
            "Họ và tên": "TRẦN THỊ B",
            "Ngày sinh": "15/05/1995",
            "Giới tính": "Nữ",
            "Quốc tịch": "Việt Nam",
            "Nơi cư trú": "Ba Đình, Hà Nội",
            "Độ tin cậy AI": "98.5%"
        };
        document.getElementById('raw-data-result').value = JSON.stringify(dummyData, null, 2);
    }, 2000);
